...
from surftg.helper.index import get_files, posts_file, posts_tmdb, encode
...
@routes.get('/static/{chat_id}/{name}', allow_head=True)
async def wstream_handler_watch(request: web.Request):
    print("start")
    try:
        chat_id = request.match_info['chat_id']
        name = request.match_info['name']
        data = await db.search_static_tgfiles(chat_id, name)
        hash_value = data[0].get('hash')
...
@routes.get('/player/{chat_id}/{name}', allow_head=True)
async def wsstream_handler_watch(request: web.Request):
    print("start")
    try:
        chat_id = request.match_info['chat_id']
        name = request.match_info['name']
        ...
        db_id = str(data[0].get('_id'))
        chat_id = f"-100{chat_id}"
        return web.Response(text=await render_page(msg_id, hash_value, chat_id=chat_id, route="player", file_name=name, file_size=size, db_id=db_id), content_type='text/html')
    except InvalidHash as e:
        raise web.HTTPForbidden(text=e.message) from e
    except (AttributeError, BadStatusLine, ConnectionResetError):
        pass
...
@routes.get('/tg/{chat_id}/{name}', allow_head=True)
async def wssxtream_handler_watch(request: web.Request):
    print("start")
    try:
        chat_id = request.match_info['chat_id']
        name = request.match_info['name']
...
@routes.get('/swatch/{chat_id}/{encoded_name}', allow_head=True)
async def ssastatic_stream_handler(request: web.Request):
    try:
        chat_id = request.match_info['chat_id']
        name = request.match_info['encoded_name']
        data = await db.search_static_tgfiles(chat_id, name)
        ...
        return await media_streamer(request, int(chat_id), int(msg_id), hash_value)
    except InvalidHash as e:
        raise web.HTTPForbidden(text=e.message) from e
    except FIleNotFound as e:
        db.delete_file(chat_id=chat_id, msg_id=data.msg_id, hash=data.hash)
        raise web.HTTPNotFound(text=e.message) from e
    except (AttributeError, BadStatusLine, ConnectionResetError):
        pass
    except Exception as e:
